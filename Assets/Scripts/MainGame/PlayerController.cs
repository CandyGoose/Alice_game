...
public class PlayerController : MonoBehaviour {
	...
	void Start () {
        ...
        dialogueBox = FindAnyObjectByType<TextBoxManager>();
	...
	void Update () {
        ...
        if (canMove )
        {
            if (knockBackCounter <= 0)
            {
                ...
                {
                    playerBody.linearVelocity = new Vector3(moveSpeed, playerBody.linearVelocity.y, 0f); // keep the y speed same as it is, and make the z 0 because no need to use z in 2D game.
                ...
                {
                    playerBody.linearVelocity = new Vector3(0f, playerBody.linearVelocity.y, 0f);//stop when no input.
                }

                if (Input.GetButtonDown("Jump") && isGrounded)
                {

                    playerBody.linearVelocity = new Vector3(playerBody.linearVelocity.x, jumpSpeed, 0f); //the x axis should stay the same to add direction to the jump.
                }
        ...
        if (knockBackCounter > 0)
        {
            ...
            if (transform.localScale.x > 0)
            {
                ...
                playerBody.linearVelocity = new Vector3(-knockBackForce, knockBackForce, 0f);
            }
            else
            {
        ...
        myAnim.SetFloat("Speed", Mathf.Abs(playerBody.linearVelocity.x)); //math function.abs gives the absolute value.[unsigned]
